...
def visualize_data(trn_data_dir, img_per_class=6):    
    ...
    # 한 번 load 할 때는 iter에 넣고, next()를 이용해서 한장씩 꺼내서 사용가능
    img, label = iter(temp_dloader).next()
    
    # 로드한 이미지들의 class 값을 구하기(label에 있는 값 중에 중복을 제거)
...
# ## 미리 학습한 weight을 이용해서 test를 수행합니다.
...
# ## Test image를 하나씩 돌려보면서 similarity 계산을 확인합니다.

# In[87]:


# Read paired images
fixed_test_pair = [l.split(" ") for l in fixed_test_pair]
print(fixed_test_pair)
...
sample_num = 0
path1 = fixed_test_pair[sample_num][0]
path2 = fixed_test_pair[sample_num][1]
print(fixed_test_pair[sample_num][2])

img_1 = Image.open(os.path.join('../ArcFace data/lfw', path1))
img_2 = Image.open(os.path.join('../ArcFace data/lfw', path2))
...
x_1 = t(img_1).unsqueeze(dim=0).to(dev)
x_2 = t(img_2).unsqueeze(dim=0).to(dev)

feature_1 = net.extract_feature(x_1).cpu()
feature_2 = net.extract_feature(x_2).cpu()
cos_dist(feature_1, feature_2).item()
